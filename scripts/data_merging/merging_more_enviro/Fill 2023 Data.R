...
temp_avg_output_reformat <- temp_avg_output_reformat %>%
  ...
  select(year,month,lat,lon,temperature_depth_105,temperature_depth_280,temperature_depth_55)
...
# Load CalCoFI data
setwd("~/Desktop/2025 Winter/PSTAT 197B/capstone-scripps/data/Merged_CalCOFI/Imputed_Data")
cal_data <- read_csv("CalCOFI_merged_2012-2023.csv")
cal_data <- cal_data %>% mutate(month = as.numeric(month))

cal_data_subset<-cal_data %>% filter(year == 2023) %>% 
  select(-pressure_depth_280, -pressure_depth_105, -pressure_depth_55) %>%
  select(-temperature_depth_55,-temperature_depth_105,-temperature_depth_280)
...
Environ_list<-list(pressure_list, temp_list)
...
for (var in Environ_list) {
  ...
  for (col in col_names) {
    Cal_COFI_sub_merge <- copy_by_next_valid_nearest(df, cal_data_subset, col, k = 10)
    cal_data_subset<- Cal_COFI_sub_merge
  }
}

vars_to_overwrite <- c("temperature_depth_55", "temperature_depth_105", "temperature_depth_280",
                       "pressure_depth_55","pressure_depth_105","pressure_depth_280")

cal_data_subset_update <- cal_data_subset %>% 
  select(lat, lon, month, all_of(vars_to_overwrite))

cal_data_updated <- cal_data %>%
  left_join(
    cal_data_subset_update,
    by = c("lat", "lon", "month"),
    suffix = c("", ".new")
  ) %>%
  mutate(across(
    all_of(vars_to_overwrite),
    ~ if_else(year == 2023, get(paste0(cur_column(), ".new")), .),
    .names = "{.col}"
  )) %>%
  select(-ends_with(".new"))

cal_data_updated_unique <- cal_data_updated %>%
  distinct(lat, lon, year, month, .keep_all = TRUE)

# write into csv file
setwd("~/Desktop/2025 Winter/PSTAT 197B/capstone-scripps/data/Merged_CalCOFI")
write_csv(cal_data, "CalCOFI_temp_merged_2012-2023.csv")
