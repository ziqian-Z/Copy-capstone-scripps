...
files <- list.files("Pressure_desired", pattern = "^Pressure_depth_.*\\.csv$", full.names = TRUE) #create a datasets folder in the current directory
...
for (i in seq_len(nrow(groups))) {
  ...
  out_file <- sprintf("output/pressure_avg_depth_%s_month_%s.csv", depth, ym)
