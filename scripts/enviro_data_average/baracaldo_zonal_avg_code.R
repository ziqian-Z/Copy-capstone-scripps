...
files <- list.files("Zonal_velocity", pattern = "^Zonal_velocity_depth_.*\\.csv$", full.names = TRUE) #create a datasets folder in the current directory

# Loop through each file and rename
for (file in files) {
  if (grepl("data", file)) {
    # Replace 'data' with 'date' in the filename
    new_name <- sub("data", "date", file)
...
dir.create("Zonal_avg", showWarnings = FALSE)
...
for (i in seq_len(nrow(groups))) {
  ...
  out_file <- sprintf("Zonal_avg/zonal_velocity_avg_depth_%s_month_%s.csv", depth, ym)
