...
files <- list.files("Temperature_desired", pattern = "^Temperature_depth_.*\\.csv$", full.names = TRUE) #create a datasets folder in the current directory
...
for (i in seq_len(nrow(groups))) {
  ...
  out_file <- sprintf("output/temperature_avg_depth_%s_month_%s.csv", depth, ym)
